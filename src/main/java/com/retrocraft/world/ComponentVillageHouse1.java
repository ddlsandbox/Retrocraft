...
public class ComponentVillageHouse1 extends StructureVillagePieces.Village
{

  private static final int HOUSE1_X = 17;
  private static final int HOUSE1_Y = 14;
  private static final int HOUSE1_Z = 13;

  private static final ResourceLocation VILLAGE_HOUSE1_ID = new ResourceLocation(RetroCraft.modId,
      "house1");

  public ComponentVillageHouse1(StructureVillagePieces.Start start, int type, StructureBoundingBox boundingBox,
      EnumFacing facing)
  {
    super(start, type);
  ...
  public boolean addComponentParts(World world, Random rand, StructureBoundingBox sbb){
      if(this.averageGroundLvl < 0){
          ...
          this.boundingBox.offset(0, this.averageGroundLvl-this.boundingBox.maxY+HOUSE1_Y-2, 0);
      }

      this.fillWithBlocks(world, sbb, 0, 0, 0, HOUSE1_X-1, HOUSE1_Y-1, HOUSE1_Z-1, Blocks.AIR);
      this.spawnActualHouse(world, sbb);
      //this.fillHouse(world, sbb);

      for(int i = 0; i < HOUSE1_X; i++){
          for(int j = 0; j < HOUSE1_Z; j++){
              this.clearCurrentPositionBlocksUpwards(world, i, HOUSE1_Y, j, sbb);
  ...
  private void spawnActualHouse(World world, StructureBoundingBox sbb){
    ...
    if(manager != null && server != null){
        ...
        Template template = manager.getTemplate(server, VILLAGE_HOUSE1_ID);
      ...
      EnumFacing facing, int type)
  {
    StructureBoundingBox boundingBox = StructureBoundingBox.getComponentToAddBoundingBox(x, y, z, 0, 0, 0,
        HOUSE1_X, HOUSE1_Y, HOUSE1_Z, facing);
    if (canVillageGoDeeper(boundingBox) && findIntersecting(pieces, boundingBox) == null)
    {
      return new ComponentVillageHouse1(startPiece, type, boundingBox, facing);
    }
