...
public class ComponentVillageTower extends StructureVillagePieces.Village
{

  private static final int TOWER_X = 17;
  private static final int TOWER_Y = 28;
  private static final int TOWER_Z = 17;

  private static final ResourceLocation VILLAGE_TOWER_ID = new ResourceLocation(RetroCraft.modId,
      "tower");

  public ComponentVillageTower(StructureVillagePieces.Start start, int type, StructureBoundingBox boundingBox,
      EnumFacing facing)
  {
    super(start, type);
  ...
  public boolean addComponentParts(World world, Random rand, StructureBoundingBox sbb){
      if(this.averageGroundLvl < 0){
          ...
          this.boundingBox.offset(0, this.averageGroundLvl-this.boundingBox.maxY+TOWER_Y-2, 0);
      }

      this.fillWithBlocks(world, sbb, 0, 0, 0, TOWER_X-1, TOWER_Y-1, TOWER_Z-1, Blocks.AIR);
      this.spawnActualHouse(world, sbb);
      //this.fillHouse(world, sbb);

      for(int i = 0; i < TOWER_X; i++){
          for(int j = 0; j < TOWER_Z; j++){
              this.clearCurrentPositionBlocksUpwards(world, i, TOWER_Y, j, sbb);
  ...
  private void spawnActualHouse(World world, StructureBoundingBox sbb){
    ...
    if(manager != null && server != null){
        ...
        Template template = manager.getTemplate(server, VILLAGE_TOWER_ID);
      ...
      EnumFacing facing, int type)
  {
    StructureBoundingBox boundingBox = StructureBoundingBox.getComponentToAddBoundingBox(x, y, z, 0, 0, 0,
        TOWER_X, TOWER_Y, TOWER_Z, facing);
    if (canVillageGoDeeper(boundingBox) && findIntersecting(pieces, boundingBox) == null)
    {
      return new ComponentVillageTower(startPiece, type, boundingBox, facing);
    }
