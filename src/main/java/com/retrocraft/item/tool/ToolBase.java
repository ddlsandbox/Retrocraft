...
public class ToolBase extends ItemPickaxe {
    ...
    public static RayTraceResult raytraceFromEntity (World world, Entity player, boolean par3, double range) {
        ...
        float f8 = f3 * f5;
        double d3 = range;
        if (player instanceof EntityPlayerMP) d3 = ((EntityPlayerMP) player).interactionManager.getBlockReachDistance();
    ...
    protected void breakExtraBlock(World world, BlockPos blockPos, EnumFacing sidehit, EntityPlayer playerEntity,
			BlockPos refPos) {
		...
		if (!world.isRemote) {
			...
			if (block.removedByPlayer(blockState, world, blockPos, player, true)) {
				block.onBlockDestroyedByPlayer(world, blockPos, blockState);
				block.harvestBlock(world, player, blockPos, blockState, world.getTileEntity(blockPos), player.getHeldItemMainhand());
