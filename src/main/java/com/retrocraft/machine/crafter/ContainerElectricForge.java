...
public class ContainerElectricForge extends Container
{

  public final World             world;
  public final InventoryCrafting craftMatrix = new InventoryCrafting(this, 3,
      3);
  public final IInventory        craftResult = new InventoryCraftResult();

  public ContainerElectricForge(EntityPlayer player)
  {
    InventoryPlayer inventory = player.inventory;

    this.world = player.world;

    this.addSlotToContainer(new SlotCrafting(inventory.player, this.craftMatrix,
    ...
    for (int i = 0; i < 3; i++)
    {
      for (int j = 0; j < 3; j++)
      {
        this.addSlotToContainer(
            new Slot(this.craftMatrix, j + i * 3, 30 + j * 18, 17 + i * 18));
      }
    }
  ...
  public ItemStack transferStackInSlot(EntityPlayer player, int index)
  {
    ...
    if (slot != null && slot.getHasStack())
    {
      ...
      if (StackUtil.getStackSize(itemstack1) == StackUtil
          .getStackSize(itemstack))
      {
        return StackUtil.getNull();
      }
