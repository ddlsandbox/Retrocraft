...
public class GuiEnchantorium extends GuiContainer {
	...
	private ArrayList<GuiLabel> enchantmentArray = new ArrayList<GuiLabel>();
	private Map<Integer, Integer> enchantments;
	
	private static final ResourceLocation BG_TEXTURE = new ResourceLocation(RetroCraft.modId, "textures/gui/enchantorium.png");
	private ContainerEnchantorium container;
	...
	public GuiEnchantorium(Container container, InventoryPlayer playerInv) {
		super(container);
		...
		this.container = (ContainerEnchantorium) container;
	}
     ...
     */
    private ArrayList<GuiLabel> convertMapToGuiItems (final Map<Integer, Integer> map, int x, int y) {
        
        final ArrayList<GuiLabel> temp = new ArrayList<GuiLabel>();
...
//        for (i=0; i<10; i++) {
            temp.add(new GuiLabel(container, obj, map.get(obj), x, yPos));
//        	temp.add(new GuiLabel(container, i, i, x, yPos));
...
//								ENCHANTLIST_Z_MIN + guiTop);
		...
		for (final GuiLabel item : enchantmentArray) {
	...
	private GuiLabel getItemFromPos (int x, int y) {
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		String name = RetroCraft.proxy.localize(ModBlocks.blockEnchantorium.getUnlocalizedName() + ".name");
...
//                EnchantingPlus.network.sendToServer(new PacketGui(player.getDisplayName(), 1, x, y, z));
    ...
    private void handleChangedEnchantment (Map<Integer, Integer> enchantments, GuiLabel item) {
    ...
    private void enableEnchantments () {
        ...
        for (GuiLabel item : enchantmentArray) {
            item.locked = false;
        }
        
        for (GuiLabel item : enchantmentArray) {
            if (item.enchantmentLevel != 0) {
                for (final GuiLabel item2 : enchantmentArray) {
                    if (item == item2)
                        continue;
